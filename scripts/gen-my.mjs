...
const SKIP_KEYS = new Set([
  'lang', 'langLabel', 'altLangLabel',
  'githubUrl', 'xUrl', 'email', 'applicantsEmail', 'sponsorshipEmail',
  'privacyEmail', 'cocEmail', 'githubHandle', 'xHandle', 'githubLabel',
]);

// Values that look like a bare URL or email are never wrapped.
const isLiteral = (v) =>
  /^https?:\/\//.test(v) || /@[\w.-]+\.\w+$/.test(v.trim()) || /^@[\w-]+$/.test(v.trim());
